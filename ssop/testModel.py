...
class TestProblem:
    def __init__(self, name="testProblem", initx=[0, 0, 0], dim=3, shiftDelta=0.1, shiftSigns=[1, 1, 1], extName='', debug=False, options=None, model_options=''):
        self.name = name
        self.model = ConcreteModel(name)
        # Dimension of variable space
        self.model.dim = dim
        self.model.idx = RangeSet(1, dim)
        ...
        self.model.cons_objvar = Constraint(rule=cons_objvar_rule)
        ...
        def ConsUp_rule(model, i):
            if shiftSigns[i-1] > 0:
                ...
                exp = (model.x[i] <= 1.)
            return exp
        self.model.ConsUp = Constraint(self.model.idx, rule=ConsUp_rule)
        ...
        def ConsLow_rule(model, i):
            if shiftSigns[i-1] < 0:
                ...
                exp = (-1              <= model.x[i])
            return exp
        self.model.ConsLow = Constraint(self.model.idx, rule=ConsLow_rule)
