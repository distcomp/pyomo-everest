...
def get_token(server_uri, user, password, label):
    r = requests.post(
        server_uri + "/auth/access_token",
...
class Job:
    ...
    def fromjson(cls, session, job_json):
        ...
        return cls(session, job_json['name'], job_json['app'], job_json['inputs'],
...
''')
    ...
    def get_token(self):
        ...
        token = get_token(args.server_uri, args.user, pswd, args.label)
        print(token)
